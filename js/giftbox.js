...
export function initGift({ scene, camera, registerUpdate }) {
  ...
  const spotlight = new THREE.SpotLight(0xfff0d0, 0.4, 8, Math.PI / 7, 0.5, 1.2);
  ...
  // The pendant setting is a ring sized LARGER than the gem so the
  // gem actually nests inside it, rather than sticking out past it.
  const pendantSetting = new THREE.Mesh(new THREE.TorusGeometry(0.078, 0.02, 12, 32), wireMat.clone());
  pendantSetting.position.y = -0.4;
  jewelryGroup.add(pendantSetting);

  // The gem itself: a rounded, many-faceted cut (icosahedron) instead
  // of a plain octahedron — an octahedron reads as a flat 4-point star
  // from most angles, this reads as an actual cut gemstone.
  const gem = new THREE.Mesh(new THREE.IcosahedronGeometry(0.058, 0), diamondMat);
  gem.position.y = -0.4;
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const prong = new THREE.Mesh(new THREE.SphereGeometry(0.012, 8, 8), prongMat);
    prong.position.set(Math.cos(a) * 0.07, -0.4 + 0.03, Math.sin(a) * 0.07);
  ...
  const observer = new MutationObserver(() => {
    if (stageEl.classList.contains('active')) {
      ...
      camera.rotation.set(-0.14, 0, 0); // slight downward tilt — pedestal sits lower now, keeps it framed below the text
      gsap.fromTo(camera.position, { y: 1.1, z: 6.2 }, { y: 0.35, z: 3.6, duration: 3, ease: 'power2.out' });
      if (!materialized) {
